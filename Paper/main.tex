...
\textbf{Semantics.} First of all two assumptions are important:\begin{enumerate}
    \item There is a finite sequence of occurrences called $\sigma$ that is a merge of the system registered logs organized by ascending time. An example of event could be the following: 
    \begin{align*}
        & \text{LoginLogoutEvent}\{userId:\ \underline{\text{string}},action:\ \underline{\text{int}}, userId:\ \time{\text{double}}\} 
    \end{align*}
\end{enumerate}
