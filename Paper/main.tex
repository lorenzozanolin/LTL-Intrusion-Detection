...
\textbf{Calculus.} According to~\cite{barringer2004rule}, the definitions of $evaluate$, $update$ and $value$ on the different primitive operators are the following ($op\text{ can be }\lor , \land, \rightarrow $):
%evaluate
\begin{align*}
...
\begin{align*}
    &evaluate(M_3,\alpha_1)  = \text{EvTimedLogout("Lori",17.0, 100)} \land \\ 
    &\quad\quad\quad\quad \text{Always}((action = \text{login}) \rightarrow \text{EvTimedLogout}(userId,time, 100))
\end{align*}
