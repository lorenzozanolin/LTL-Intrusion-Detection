...
As we can see, rules are parameterized, which means that we will be able to write EAGLE formulas such as $\text{Always}(\text{Sometime}(x>0))$. Also, the $\text{Always}$ operator is defined as maximal solution of the equation $X=F\land \ocircle X$ and the $\text{Sometime}$ operator represents the minimal solution to the equation $X=F\lor \ocircle X$.
